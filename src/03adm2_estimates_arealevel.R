...
rq_packages <- c("readr", "readxl","tidyverse","survey","srvyr", "sf", "spdep", "tmap", "INLA", "grid",
                 "SUMMER", "wesanderson", "cmdstanr", "ggplot2", "ggrepel","ggpubr","viridis")
...
nga_analysis_df <- nga_base_ai %>% 
  ...
  left_join(hh_info %>% dplyr::select(hhid, ea, survey_wgt,age_head,sex_head) |> mutate(female_head = case_when(sex_head=="Female"~1,.default = 0)) |> select(-sex_head), by = "hhid")
...
# main indicator 
nutrient = "folate"
model_name = "area_level_joint_smooth_logit"
runname = "20250717-01"
...
df_process <- sae_df(nga_analysis_df, nutrient=nutrient, geo_level="lga", geo_level_upper = "state", add_phantom = T)
...
data_list <- list(
  N = nrow(inad_lga),
  NS = nrow(inad_lga_complete),
  adm2_index = inad_lga_complete$lga_id,
  p_hat = inad_lga_complete$design_based_ph_mean,
  v_hat = inad_lga_complete$design_based_ph_var,
  d = inad_lga_complete$degf_ph, 
  k = inad_lga_complete$n_obs_ph,
...
fit$save_output_files(dir = folder_path, basename = "stan_postsample")
csv_files <- fit$output_files()
saveRDS(csv_files,here('outputs','mcmc',paste0('csv_files_postsample_vb12_',model_name,'.rds')))
...
inad_lga$design_based_ph_cv <- inad_lga$design_based_ph_var / sqrt(inad_lga$design_based_ph_mean)

inad_lga$naive_mean100 <- inad_lga$naive_mean * 100
inad_lga$design_based_mean100 <-inad_lga$design_based_mean * 100
inad_lga$design_based_ph_mean100 <-inad_lga$design_based_ph_mean * 100
...
tmap_save(naive_map,
          here("outputs","maps",paste0(nutrient,"_inad_naive_",model_name,".png")), 
          paste0("outputs/maps/", nutrient, "_inad_naive.png"), 
          width = 8, height = 8,
          units = "in", dpi = 600)
...
direct_map
tmap_save(direct_map, 
          here("outputs","maps",paste0(nutrient,"_inad_direct_",model_name,".png")), 
          width = 8, height = 8,
          units = "in", dpi = 600)

direct_map <- plot_map(data = inad_lga , 
                       col = "design_based_ph_mean100", 
                       title = paste0(str_to_title(nutrient)," (design-based estimates)"), 
                       metric = "Prevalence of inadequate intake (%)", 
                       level = "lga")
...
FB_map
tmap_save(FB_map, 
          here("outputs","maps",paste0(nutrient,"_inad_FB_",model_name,".png")), 
          width = 8, height = 8,
          units = "in", dpi = 600)

inad_lga$post_mean_obs_only = inad_lga$post_mean
inad_lga$post_mean_obs_only[!inad_lga$lga_id%in%inad_lga_complete$lga_id] = NA
inad_lga$post_mean_obs_only100 <- inad_lga$post_mean_obs_only *100
FB_map <- plot_map(data = inad_lga,
                   col = "post_mean_obs_only100", 
                   title = "Folate (Full-Bayes BYM2 smoothed estimates)", 
                   metric = "Prevalence of inadequate intake (%)", 
                   level = "lga")

FB_map
tmap_save(FB_map, 
          here("outputs","maps",paste0(nutrient,"_inad_FB_",model_name,"_obs_only.png")), 
          width = 8, height = 8,
          units = "in", dpi = 600)
...
vb12_cv_ph <- plot_map_ggplot(data = inad_lga, 
                              col = "design_based_ph_cv", 
                              title = "CV of direct weighted estimates", 
                              metric = "Variance")
