...
vb12_inad_lga$v12_inad_post_mean <- df_p[,1:nrow(vb12_inad_lga)] |> colMeans() |> unname()
vb12_inad_lga$v12_inad_post_var <-  sapply(df_p[,1:nrow(vb12_inad_lga)],var) |> unname()
...
FB_map <- plot_map(data = vb12_inad_lga ,
                       col = "v12_inad_post_mean", 
                       title = "Vitamin B12 (Full-Bayes BYM2 smoothed estimates)", 
                       metric = "Prevalence of inadequate intake (%)", 
                       level = "lga")

FB_map
tmap_save(FB_map, "outputs/maps/vb12_inad_FB_BYM2.png", width = 8, height = 8,
          units = "in", dpi = 600)
