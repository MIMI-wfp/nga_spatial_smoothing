...
if (model_name=='BYM2') run = TRUE else run = FALSE 
# ------------------------------------------------------------------------------
# Stan
stan_csv_filenames <- readRDS(here('outputs','mcmc',here('csv_files_postsample_vb12_',model_name,'.rds')))
...
# set `run = FALSE` if no artificial data should be added
if (run){
...
vb12_inad_lga <- vb12_inad_lga |> 
  left_join( nga_analysis_df |> group_by(state,lga) |>
               summarise(n_obs = n(),
...
vb12_inad_state <- nga_analysis_df |> group_by(state) |>
  summarise(n_obs = n(),
...
df_post <- cbind(vb12_inad_lga |> select(state,lga,sum_wgt),
                 t(df_p[,1:nrow(vb12_inad_lga)])) |> 
...
# If using population proportion for weights
...
cor(vb12_inad_state$vb12_inad_wdir, vb12_inad_state$vb12_inad_post_mean, method = "pearson")
...
write_csv(vb12_inad_state, here("processed_data",paste0("state_level_estimates",model_name,".csv")))
#-------------------------------------------------------------------------------
# Plots
#-------------------------------------------------------------------------------
...
p
ggsave(here("outputs","maps",paste0("vb12_inad_state_FB",model_name,".png")), p, width = 10, height = 10,bg = "white")
...
p = ggplot(vb12_inad_state |> st_as_sf()
  ...
  theme_minimal(base_size = 14) 
p
ggsave(here("outputs","maps",paste0("vb12_inad_state_coverage",model_name,".png")), p, width = 10, height = 10,bg = "white")
...
p = ggplot(vb12_inad_state |> st_as_sf()
  ...
  theme_minimal(base_size = 14) 
p
ggsave(here("outputs","maps",paste0("vb12_inad_state_error",model_name,".png")), p, width = 10, height = 10,bg = "white")
...
p <- ggplot(vb12_inad_state,aes(x=vb12_inad_post_mean,y=vb12_inad_wdir,label = str_to_title(state))) +
  ...
  theme_minimal(base_size = 14)
p
ggsave(here("outputs","maps",paste0("vb12_inad_state_scatter",model_name,".png")), p, width = 10, height = 10,bg = "white")
