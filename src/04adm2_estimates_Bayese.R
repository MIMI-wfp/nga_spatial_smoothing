...
stanfile <- here('src',paste0('areal_level_',model_name,'.stan')) # For Binomial model, "areal_level_binom_BYM2.stan"
...
fit$save_output_files(dir = here('outputs','mcmc'), basename = paste0("stan_postsample_vb12_",model_name))
...
vb12_inad_lga$vb12_inad_post_mean_obs_only = vb12_inad_lga$vb12_inad_post_mean
vb12_inad_lga$vb12_inad_post_mean_obs_only[!vb12_inad_lga$lga_id%in%vb12_inad_lga_complete$lga_id] = NA
FB_map <- plot_map(data = vb12_inad_lga,
                   col = "vb12_inad_post_mean_obs_only", 
                   title = "Vitamin B12 (Full-Bayes BYM2 smoothed estimates)", 
                   metric = "Prevalence of inadequate intake (%)", 
                   level = "lga")

FB_map
tmap_save(FB_map, here("outputs","maps",paste0("vb12_inad_FB_",model_name,"_obs_only.png")), width = 8, height = 8,
          units = "in", dpi = 600)
