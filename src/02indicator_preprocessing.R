...
rwa_base_ai <- apparent_intake("/rwa_eicv2324")

# Tidy environment: 
rm(list = setdiff(ls(), c("nga_base_ai","rwa_base_ai", "allen_ear")))
...
# Same for Rwanda: 
rwa_base_ai$folate_inadequate <- ifelse(rwa_base_ai$folate_mcg < allen_ear$ear_value[allen_ear$nutrient == "folate_mcg"], 1, 0)
rwa_base_ai$vb12_inadequate <- ifelse(rwa_base_ai$vitb12_mcg < allen_ear$ear_value[allen_ear$nutrient == "vitb12_mcg"], 1, 0)

rwa_base_ai <- rwa_base_ai %>% 
  dplyr::select(hhid, folate_inadequate, vb12_inadequate)
...
# Tidy environment: 
rm(list = setdiff(ls(), c("nga_base_ai", "rwa_base_ai")))
